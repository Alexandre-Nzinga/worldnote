...
const variantConfig = {
    ...
    danger: {
        heroVariant: "solid",
        className: "border-0 !bg-wn-red-500 font-semibold text-wn-mono-50 shadow-none hover:!bg-wn-red-600 data-[hover=true]:!bg-wn-red-600",
    },
};
const sizeClass = {
...
export function Button({ variant = "primary", size = "base", children, className, isDisabled, isIconOnly, fullWidth, ...props }) {
    const config = variantConfig[variant];
    const isLink = variant === "link";
    const reducedMotion = usePrefersReducedMotion();
    const canAnimate = !isDisabled && !isLink && !reducedMotion;
    const isFullWidth = Boolean(fullWidth) && !isLink && !isIconOnly;
    return (_jsx(motion.span, { className: clsx("inline-flex shrink-0", isLink && "w-auto", isFullWidth && "w-full max-w-full"), whileTap: canAnimate ? pressableTap : undefined, transition: tapTransition, children: _jsx(HeroUIButton, { ...props, fullWidth: fullWidth, isDisabled: isDisabled, variant: config.heroVariant, color: config.heroColor, size: heroUiSize[size], radius: "full", classNames: {
                base: "gap-2",
            }, className: clsx(!isLink && sizeClass[size], config.className, className), children: children }) }));
}
