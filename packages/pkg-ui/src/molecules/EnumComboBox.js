...
const triggerClassName = "relative h-10 min-h-10 rounded-full border border-wn-mono-700 bg-wn-mono-950 px-3 pr-9 shadow-none data-[hover=true]:border-wn-mono-600 data-[hover=true]:bg-wn-mono-950 group-data-[focus=true]:border-wn-mono-500";
const valueClassName = "w-full min-w-0 truncate text-left text-sm text-wn-mono-50 group-data-[has-value=true]:text-wn-mono-50";
...
export function EnumComboBox({ id, label, value, options, onChange, disabled = false, placeholder = "Select…", allowEmpty = false, className, }) {
    ...
    return (_jsxs("div", { className: `flex flex-col gap-1 ${className ?? ""}`, children: [_jsx("label", { htmlFor: id, className: fieldLabelClassName, children: label }), _jsx(Select, { id: id, "aria-label": label, placeholder: hasSelection ? undefined : placeholder, items: items, selectedKeys: selectedKeys, isDisabled: disabled, disallowEmptySelection: !allowEmpty, renderValue: (selected) => {
                    const item = selected[0];
                    if (!item) {
                        return null;
                    }
                    const data = item.data;
                    return (_jsx("span", { className: "block w-full truncate text-left", children: data?.label ?? item.textValue }));
                    ...
                    selectorIcon: "pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-wn-mono-400",
